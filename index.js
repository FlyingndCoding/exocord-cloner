...
(async function () {
  setTitle("ExoCord Server Cloner | Setup");
  console.log(" ");
  console.log(" ");
  console.log(
    center(
...
`.blue,
  ...
  client.on("ready", async () => {
    ...
    let region = allowedRegions.includes(guild2.region)
      ? guild2.region
      : "eu-central";

    let guildname = guild1.name + " - Exocord";
    ...
    categories.forEach(async (category) => {
      ...
      await guild2
        .createChannel(category.name, "category")
    ...
    for (var i = 0; i < channels.length; i++) {
      ...
      if (do1.parent) {
        ...
        if (ll) {
          channel.setParent(
            guild2.channels.find((c) => c.name === do1.parent.name).id
          );
        } else {
      ...
      for (var i = 0; i < voice.length; i++) {
        ...
        if (do1.parent) {
          ...
          var ll = await guild2.channels.find(
            (c) => c.name === do1.parent.name
          );
          if (ll) {
            channel.setParent(
              guild2.channels.find((c) => c.name === do1.parent.name).id
